...
export class WeixinRuntime {
  ...
  /**
   * 通用告警/通知发送（2026-08-23）— 给 owner 发任意文本，不要求 runtime ready。
   * 供 harness 内任意系统（量化、监控等）经 controller.sendMessage 调用；
   * 发送走 sendmessage API，只要 token 有效即可送达（无需 bot 长轮询在线）。
   * 失败抛错（由 controller 转成统一错误结构），调用方决定是否重试。
   */
