...
export class WeixinController {
  ...
  /**
   * 通用告警/通知发送（2026-08-23）— 对任一已配置 bot 给 owner 发文本。
   * 供 harness 内任意系统（量化、监控等）调用；不要求 runtime ready
   * （sendToOwner 直接走 sendmessage API）。无已配置账号 → 抛错。
   */
  async sendMessage(text) {
    const configured = this.#configStore.list();
    if (configured.length === 0) throw connectionTestTargetUnavailable('微信机器人');
    let lastError;
    for (const config of configured) {
      const runtime = this.#runtimes.get(config.botId);
      if (!runtime || typeof runtime.sendToOwner !== 'function') continue;
      try {
        return await runtime.sendToOwner(String(text));
      } catch (error) {
        lastError = error;
      }
    }
